 ...
 */
function itemPricingSuitelet(request, response){
	...
	if (request.getMethod() == 'GET') 
	{
		...
		//=====================================================================
		//
		var form = nlapiCreateForm('Create Customer Item Pricing', false);
		form.setScript('customscript_bbs_item_pricing_cl');
		form.setTitle('Create Customer Item Pricing');
		...
		switch(stageParam)
		{
					...
					for ( var baseParentId in baseParents) 
						{
							...
							sublist.addButton(sublistId + '_update_prices', 'Update Prices', buttonFunction2);
			...
			case 3:
					...
					messageField.setDefaultValue('An email will be sent to ' + emailAddress + ' when the customer item pricing creation process has completed.');
	...
	else
	{
		...
		switch(stage)
		{
			...
			case 2:
				...
				//
				var scheduleParams = {
							custscript_bbs_cip_parent_child: JSON.stringify(parentAndChild), 
							custscript_bbs_cip_customer_id: custIdParam
							};
				
				nlapiScheduleScript('customscript_bbs_item_pricing_sh', null, scheduleParams);
