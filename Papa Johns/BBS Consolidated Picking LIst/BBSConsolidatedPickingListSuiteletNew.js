...
function consolidatedPickingSuitelet(request, response)
{
	...
	if (request.getMethod() == 'GET') 
	{
		...
		routeField.setHelpText('Enter the route code to print, or leave blank for all applicable routes', true);
	...
	else
	{
		...
		//See if we have a blank route code, if so we need to find all the relevant routes
		//
		var routesArray = {};
		
		if(routeCode == '')
			{
				var routesSearch = nlapiSearchRecord("salesorder",null,
						[
						   ["type","anyof","SalesOrd"], 
						   "AND", 
						   ...
						   "AND", 
						   ["shipdate","within",nlapiDateToString(deliveryDateFrom),nlapiDateToString(deliveryDateTo)]
						], 
						[
				...
				if(routesSearch)
					{
						for (var int = 0; int < routesSearch.length; int++) 
							{
								var routeCodeResult = routesSearch[int].getValue("custbody_pj_routeno", null,"GROUP");
								
								if(!routesArray[routeCodeResult])
									{
										routesArray[routeCodeResult] = routeCodeResult;
									}
							}
					}
			}
			...
			{
				//If we have specified a route that just add it to the routes array
				//
				routesArray[routeCode] = routeCode;
			}
		
		//Loop round the routes
		//
		for (var routeCode in routesArray) 
		{
