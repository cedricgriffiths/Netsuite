...
{
	var PORTLET_TITLE = "Appointments";
	...
	var ICON_SINGLE_GOOGLE = URL_GOOGLE_MAP + '/mapfiles/ms/micons/red.png';
 ...
 */
function getEvents(externalSearchId) {
	var unmappedCount = 0;
	var htmlUnmapped = "";
	var jsonEvents = "";
	var results = null;
	...
	if (externalSearchId != null && externalSearchId != '') {
		...
		if (externalSearchId != '0') {
			results = nlapiSearchRecord(REC_EVENT, externalSearchId, null, null);
		}
	}
	else {
		results = nlapiSearchRecord(REC_EVENT, EVENT_SEARCH, null, null);
	}
	...
	if (results) {
		...
		for (var i = 0; i < results.length; i++) {
			...
			var startdate = results[i].getValue(FLD_STARTDATE);
...
function eventGoogleMapsSuitelet(request, response) {
	...
	if (searchIdParam != null && searchIdParam != '') {
		eventsToday = getEvents(searchIdParam);
	}
	else {
		eventsToday = getEvents('');
	}
	...
	if (eventsToday.mapped.length > 0) {
		...
		if (eventsToday.unmappedCount > 0) {
...
function lookupPreviousAddress(address) {
	var results = nlapiSearchRecord(REC_EVENT, null, [ new nlobjSearchFilter(FLD_ADDRESS, null, 'is', trim(address)), new nlobjSearchFilter(FLD_LATITUDE, null, 'isnotempty', null) ], [ new nlobjSearchColumn(FLD_LATITUDE, null, null), new nlobjSearchColumn(FLD_LONGITUDE, null, null) ]);
	if (results) {
...
function portletName(portletObj, column) {
	...
	portletObj.setTitle('Google Portlet');

	portletObj.setScript('customscript_bbs_portlet_client');

	//Create the select field to pick the seach to run
	//
	var field1 = portletObj.addField('custpage_select', 'select', 'Select Search', 'customrecord_bbs_google_map_searches');
	...
	//Search the map searches entity to find the default one
	//
	var cols = new Array();
	cols[0] = new nlobjSearchColumn('custrecord_bbs_saved_search');
	cols[1] = new nlobjSearchColumn('custrecord_bbs_default_search');

	var filters = new Array();
	filters[0] = new nlobjSearchFilter('custrecord_bbs_default_search', null, 'is', 'T');

	var googleSearches = nlapiCreateSearch('customrecord_bbs_google_map_searches', filters, cols);
	var googleResults = googleSearches.runSearch();
	var googleResultset = googleResults.getResults(0, 100);

	if (googleResultset.length > 0) {

		var defaultSearchText = googleResultset[0].getText('custrecord_bbs_saved_search');
		var defaultSearchValue = googleResultset[0].getValue('custrecord_bbs_saved_search');

		//field1.setDefaultValue(defaultSearchText);
		//nlapiSetFieldValue('custpage_select', defaultSearch, true, true);

		var html = '<iframe src="' + serverUrl + '&searchid=' + defaultSearchValue + '" width="100%" align="center"  height="' + (height + 4) + 'px" style="margin:0px; border:0px; padding:0px"></iframe>';
