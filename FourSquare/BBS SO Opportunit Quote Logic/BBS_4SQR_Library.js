 ...
 * Module Description
 * 
 * Version Date Author Remarks 1.00 05 Apr 2016 cedric
 * 
 */
function LibProcessFieldChanges(type, name, linenum, unitCostField)
{
	...
	if (type == 'item')
	{
		...
		if (itemItem != null && itemItem != '')
		{
			...
			var salesMargin = Number(nlapiGetCurrentLineItemValue(type, 'custcol_bbs_4sqr_sales_margin'));
			...
			//
			if (name == unitCostField || name == 'custcol_bbs_4sqr_sales_margin')
			{
	...
	if (createdFrom != null && createdFrom != '')
	{
		...
		if (originalRecord != null)
		{
			...
			for (var lineNo = 1; lineNo <= lineCount; lineNo++)
			{
				...
				//
				var originalSupplier = originalRecord.getLineItemValue('item', 'custcol_bbs_4sqr_supplier', lineNo);
				...
				//
				var originalSupplierUnitCost = Number(originalRecord.getLineItemValue('item', 'custcol_bbs_4sqr_supp_unit_cost', lineNo));
				...
				//
				var originalSalesMargin = Number(originalRecord.getLineItemValue('item', 'custcol_bbs_4sqr_sales_margin', lineNo));

				// Update the sales margin from the original record
				//
				if (originalSalesMargin != NaN && originalSalesMargin != 0)
				{
					nlapiSetCurrentLineItemValue('item', 'custcol_bbs_4sqr_sales_margin', originalSalesMargin, false, true);
				}
